---
last_mapped: 2026-01-14T20:30:00Z
total_files: 289
total_tokens: 592834
---
...
> Auto-generated by Cartographer. Last mapped: January 14, 2026
...
│   │   │   │   └── exchange_client.py         # CCXT compatibility (4.4k)
│   │   │   └── management/commands/
...
│   ├── MIGRATION_STATUS.md          # CCXT → Native migration (818 lines)
...
**Status**: ✅ Active (Migration from CCXT complete Sept 2025)
...
NativeExchangeManager (Singleton)
    ...
    └─→ Redis listener: ccxt_requests → ccxt_responses
```
...
**Performance**: ~56% faster than CCXT (354ms vs 825ms for balance queries)
...
| `ExchangeClient` | `exchange_client.py` | 4,429 | All | CCXT compatibility wrapper |
...
```python
# Service → Terminal 5
redis.rpush('ccxt_requests', json.dumps({
    'request_id': uuid4(),
    'action': 'place_order',
    'params': {...},
...
# Terminal 5 → Service
redis.set(f"ccxt_response_{request_id}", json.dumps({
    'success': True,
    'data': {...}
}))
```
...
**Purpose**: Replace CCXT with native exchange APIs for performance
...
| `apps/core/services/exchange_client.py` | CCXT compatibility | 4,429 |
| `apps/core/management/commands/run_native_exchange_service.py` | Terminal 5 | 2,877 |

**Migration Status** (from `MIGRATION_STATUS.md`):
- ✅ Complete: Sept 8, 2025
- 📈 Performance: 56% faster (354ms vs 825ms)
- 🚀 Active: `run_native_exchange_service.py`
- ❌ Deprecated: `run_ccxt_service.py`

**Architecture Change**:
- **Before**: CCXT library, one instance per broker
- **After**: Native clients, one instance per exchange type + dynamic credentials

**Documentation**: `MODULE2-Refacto-CCXT_MicroServ.md` (6,211 tokens)
...
Frontend → TradingService.execute_trade_via_terminal5()
    ↓
ExchangeClient → Redis 'ccxt_requests'
    ↓
...
sequenceDiagram
    ...
    Terminal5->>Redis: set('ccxt_response_{request_id}')
...
sequenceDiagram
    ...
    ExchangeClient->>Redis: rpush('ccxt_requests', {request_id, action, params})

    loop Polling (0.1s interval, max 120s)
        ExchangeClient->>Redis: get('ccxt_response_{request_id}')
    end

    Terminal5->>Redis: blpop('ccxt_requests')
...
| `ccxt_requests` | Queue | Exchange action requests |
| `ccxt_response_{uuid}` | Key/Value | Exchange action responses |
...
**Issue**: `ccxt_response_{uuid}` keys accumulate in Redis.
