...
### Développement
- **Mode DEBUG** : User "dev" sans login
...
```
Contexte : Je développe Aristobot3, un bot de trading crypto en Django/Vue.js. La desription complète de Aristobot3 se trouve dans ARISTOBOT3.md. Préviens moi si tu ne peux pas le lire. Je suis le plan d'implémentation Module 1. 

Objectif : Implémenter complètement le système User Account & Brokers selon le plan.

Actions à réaliser dans l'ordre :
1. Créer les modèles dans apps/accounts/models.py et apps/brokers/models.py
2. Créer le backend d'authentification DevModeBackend
3. Créer le service CCXTService singleton
4. Créer le script management command init_aristobot
5. Créer les serializers et viewsets pour les APIs
6. Configurer les URLs
7. Créer le frontend Vue (store auth et AccountView)

Contraintes :
- PostgreSQL uniquement
- Multi-tenant strict (filtrer par user_id)
- Chiffrement avec Django SECRET_KEY
- Mode DEBUG = user "dev" sans login
- Utiliser CCXT avec enableRateLimit: true

Teste après chaque étape que le serveur démarre sans erreur.
Ne passe à l'étape suivante que si tout fonctionne.
```
...
- Tester en mode DEBUG=True et DEBUG=False
