...
4. **Terminal 5 : Exchange Gateway Centralisé**
   ...
   * **Port** : Aucun (écoute Redis)
   * **Rôle** : Le "hub" centralisé pour toutes les connexions aux exchanges. Ce service maintient une seule instance de connexion par (user_id, broker_id) et communique avec les autres services via Redis. Il garantit le respect des rate limits des exchanges et évite la multiplication des connexions.
   * **Responsabilités** :
...
### **3.3 Architecture Exchange Gateway : Service Centralisé via Redis**
...
* **Coexistence intelligente** : APIs natives directes pour tests ponctuels (User Account) + service centralisé pour opérations répétées (Trading)
...
1. Trading Engine → ExchangeClient.get_balance(broker_id)
2. ExchangeClient → Redis (exchange_requests)
3. Exchange Gateway → Traite la requête → Redis (exchange_responses)
4. ExchangeClient → Reçoit la réponse → Retourne au Trading Engine
```
...
* **Backend :** Le backend est chargé de mettre a disposition les fonctionnalités broker (Exchange) nécessaire au fonctionnement des applications Django à l'aide des APIs natives des exchanges.
    * L'**Exchange Gateway** précharge TOUS les brokers actifs au démarrage.
    * L'Exchange Gateway met à jours la DB. Les assets sont supprimés avant l'ajout des nouveaux pour être certain qu'il n'y ai pas de reliqua.
    * L'application **4.2. User Account (`apps/accounts`)** Account utilise _APIs natives directes_ pour tests de connexion (pas de rate limits). C'est une exeption !
...
* **Frontend :** 
  ...
  * Status "Chargement Exchange Gateway xxx%" durant l'initialisation
...
* **Backend** :
    ...
    * Les Exchanges (Brokers) sont fourni par les APIs natives
    * Envoie la liste des Exchanges supportés
      ```python
        # Liste des exchanges supportés nativement
        SUPPORTED_EXCHANGES = [
            'bitget',    # 40%
            'binance',   # 40% 
            'kucoin',    # 10%
    ...
    * **Utilise APIs natives directes** pour les tests de connexion et listing des exchanges (opérations ponctuelles)
    * Mise à jour des marchés via l'**Exchange Gateway centralisé** (Terminal 5)
    * Gère l'enregistrement et l'envoi des préférences utilisateur.
    * **Note technique** : User Account garde APIs natives directes car les tests de connexion sont ponctuels et ne posent pas de problème de rate limits
...
```
Contexte : Aristobot3.1, App [Nom de l'app]
...
Contraintes : [Limites techniques, ex: utiliser l'ExchangeGateway]
```